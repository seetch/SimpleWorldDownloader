...
public class ChunkMapScreen extends Screen {
    ...
    public void extractRenderState(@NonNull GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        graphics.text(this.font, activity, mapRight - this.font.width(activity), 30,
        ...
        for (int row = 0; row < rows; row++) {
            for (int column = 0; column < columns; column++) {
                ...
                if (cellSize >= 8) {
                    graphics.outline(x, y, cellSize - 1, cellSize - 1, GRID_COLOR);
                }
                if (chunkX == playerChunkX && chunkZ == playerChunkZ) {
                    graphics.outline(x, y, cellSize - 1, cellSize - 1, PLAYER_COLOR);
                    if (cellSize >= 7) {
